...
class RawRandAugment(object):
    def __init__(self,
        ...
        self.level_map = {
            ...
            "posterize": int(5.0 * abso_level),
            "solarize": 256.0 * abso_level,
            # "contrast": 0.9 * abso_level,
            "sharpness": 0.7 * abso_level,
...
def cvtColor(img):
    ...
    delta = 0.005 * random.random() * flag()
    hsv[:, :, 2] = hsv[:, :, 2] * (1 + delta)
...
def blur(img):
    ...
    if h > 10 and w > 10:
        ...
        return cv2.GaussianBlur(img, (9, 9), 1)
    else:
        return img
...
def add_gasuss_noise(image, mean=0, var=0.5):
    """
    Gasuss noise
    """

    noise = np.random.normal(mean, var**0.8, image.shape)
    out = image + 0.8 * noise
...
def warp(img, ang, use_tia=True, prob=0.4):
    ...
    if config.distort:
        ...
        if random.random() <= prob and img_height >= 20 and img_width >= 20:
            new_img = tia_distort(new_img, random.randint(3, 5))

    if config.stretch:
        img_height, img_width = img.shape[0:2]
        if random.random() <= prob and img_height >= 20 and img_width >= 20:
            new_img = tia_stretch(new_img, random.randint(3, 5))
    ...
    if config.randomaugment:
        if random.random() <= prob:
            if not isinstance(new_img, Image.Image):
                pil_img = np.ascontiguousarray(new_img)
                pil_img = Image.fromarray(pil_img)
            aug_func = RawRandAugment()
            aug_img = aug_func.__call__(pil_img)
            new_img = cv2.cvtColor(np.asarray(aug_img), cv2.COLOR_RGB2BGR)
