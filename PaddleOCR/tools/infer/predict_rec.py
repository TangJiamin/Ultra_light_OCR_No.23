...
class TextRecognizer(object):
    ...
    def __call__(self, img_list):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            norm_img_batch = []
            max_wh_ratio = self.rec_image_shape[2] * 1.0 / self.rec_image_shape[1]
        ...
        if len(img_list) >= args.rec_batch_num or idx == len(
            ...
            for ino in range(len(img_list)):
                logger.info("Predicts of {}:{}".format(valid_image_file_list[
                    ino], rec_res[ino]))
                with open('infer_pre_res.txt', 'a') as file:
                    file.write(valid_image_file_list[ino].split('/')[-1] + '\t' + list(rec_res[ino])[0] + '\n')
